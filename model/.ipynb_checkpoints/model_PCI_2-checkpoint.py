...
class VCI(nn.Module):
    ...
    def encode_ZXT_prior(self, covariates, treatments, eval=False):
        ...
        inputs = torch.cat([treatments ,covariates], -1)
    ...
    def forward(self, outcomes, treatments, contol_outcomes, covariates,
        ...
        control_outcomes_constr_samp = self.sample_control(control_latents_dist.mean, control_latents_dist.stddev, size=self.mc_sample_size
        )
    ...
    def init_encoder_XT_prior(self):
        return MLP([self.treatment_dim+self.covariate_dim]
            + [self.hparams["encoder_width"]] * (self.hparams["encoder_depth"] - 1)
