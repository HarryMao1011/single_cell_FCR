...
class VCI(nn.Module):
    ...
    def _init_indiv_model(self):
        ...
        params.extend(list(self.exp_encoder.parameters()))
...
#             raise ValueError("dist_mode not recognized")
    ...
    def encode_ZXT_prior(self, covariates, treatments, eval=False):
        ...
        inputs = torch.cat([covariates, treatments], dim=-1)
    ...
    def permutation_distribution_T(self, mu, sigma, conditions):
        ...
        # print("ZT1 shape {}".format(ZT1.shape))
        ZXT1 = sample1[:, self.ZX_dim:self.ZX_dim+self.ZXT_dim]
        ...
        ##
        # print("original shape {}, perturbed shape {}".format(original.shape, perturbed.shape))
        combined =  torch.cat([original, perturbed], 0)
        index = torch.randperm(combined.shape[0])
        combined = combined[index, :]
        # print("combined shape {}".format(combined.shape))
    ...
    def predict_self(
        ...
        with torch.autograd.no_grad():
            ...
            expr_outcomes_constr_samp = self.sample_expr(exp_dist.mean, exp_dist.stddev)
        ...
        if return_dist:
            return expr_outcomes_dist_samp
        else:
            return expr_outcomes_dist_samp.mean    
...
#         ## maximize the distance of treatment and control group
#         # (3) cosine similarities of the ZTs 
#         similarities = torch.nn.CosineSimilarity(dim=1)
#         sim_ZT =  similarities(ZT[0], control_latents_dist.mean)
...
#                 kl_divergence_ind, kl_divergence_X, kl_divergence_T, kl_divergence_XT,kl_divergence_control, conditions, conditions_labels)
    ...
    def loss_divergence(self, control_outcomes, control_outcomes_dist_samp,
            expr_outcomes, expr_outcomes_dist_samp, exp_dist, ZX_dist, ZT_dist, ZXT_dist,
            ZX_prior_dist, ZT_prior_dist, ZXT_prior_dist, control_prior_dist,control_latents_dist,
            cov_constr, treatment_constr, treatments,
            covariates, kde_kernel_std=1.0):
        ...
        # (3) cosine similarities of the ZTs 
        similarities = torch.nn.CosineSimilarity(dim=1)
        sim_ZT =  similarities(ZT_dist[0], control_latents_dist.mean[:, -self.ZT_dim:])
        sim_ZT = torch.mean(sim_ZT)
        
        sim_ZX =  similarities(ZX_dist[0], control_latents_dist.mean[:, :self.ZX_dim])
        sim_ZX = torch.mean(sim_ZX)
        ...
        return (indiv_spec_nllh_control, indiv_spec_nllh_experiments, cov_loss, treatment_loss,
                kl_divergence_ind, kl_divergence_X, kl_divergence_T, kl_divergence_XT,kl_divergence_control, conditions, conditions_labels, sim_ZT, sim_ZX)
    
    
    
    def forward(self, outcomes, treatments, contol_outcomes, covariates,
        ...
        # )
        control_latent_constr = self.encode_exp(outcomes, covariates, control_treatment)
        control_latents_dist = self.distributionize(
        ...
        ## extract ZX, ZT, ZXT with mean and stddev
        # print("ZX_dim {}, ZT_dim:{}, ZXT_dim:{}, exp_dist shape {}".format(self.ZX_dim, self.ZT_dim, self.ZXT_dim, exp_dist.mean.shape))
        ...
        ZT = (exp_dist.mean[:, -self.ZT_dim:], exp_dist.stddev[:,  -self.ZT_dim:])
        # print("ZX shape {}, ZT shape :{}, ZXT shape:{}".format(ZX[0].shape, ZT[0].shape, ZXT[0].shape))
        
        ZX_control = (control_latents_dist.mean[:, :self.ZX_dim], control_latents_dist.stddev[:, :self.ZX_dim])
        ZXT_control = (control_latents_dist.mean[:, self.ZX_dim:self.ZX_dim+self.ZXT_dim], control_latents_dist.stddev[:, self.ZX_dim:self.ZX_dim+self.ZXT_dim])
        ZT_control = (control_latents_dist.mean[:, self.ZX_dim+self.ZXT_dim:], control_latents_dist.stddev[:, self.ZX_dim+self.ZXT_dim:])
        # print("ZX_control shape {}, ZT_control shape :{}, ZXT_control shape:{}".format(ZX_control[0].shape, ZT_control[0].shape, ZXT_control[0].shape))
        ## control prior
        ## control share the same backbone encoder and prior network
        ...
        # print("ZT_resample shape {}, ZT_control_resample shape {}".format(ZT_resample.shape, ZT_control_resample.shape))
        treatment_constr = self.intervention_decode(ZT_resample, ZT_control_resample)
    ...
    def update(self, expr_outcomes, treatments, contol_outcomes, covariates, adv_training=False):
        ...
        if not adv_training: 
            ...
            loss = (self.omega0 * indiv_spec_nllh
                ...
                + 1 * kl_divergence_samples
                + 1 * kl_divergence_factored
            ...
            permute_T_loss = self.loss_discriminator_T(permute_T_pred, perturb_T[:, -1]) 
            permute_X_loss = self.loss_discriminator_X(permute_X_pred, perturb_X[:, -1])
            permute_loss =  self.omega3 * (permute_T_loss + permute_X_loss)
    ...
    def update_divergence(self, expr_outcomes, treatments, contol_outcomes, covariates, adv_training=False):
        """
        Update model's parameters given a minibatch of outcomes, treatments, and covariates.
         expriments, treatment, control, covariates, adv_training
        """
        ...
        if not adv_training: 
            ...
            kl_divergence_ind, kl_divergence_X, kl_divergence_T, kl_divergence_XT,kl_divergence_control, conditions, conditions_labels, \
            sim_ZT, sim_ZX= \
            self.loss_divergence(control_outcomes, control_outcomes_dist_samp, expr_outcomes, expr_outcomes_dist_samp, exp_dist,
                           ZX, ZT, ZXT, ZX_prior_dist, ZT_prior_dist,ZXT_prior_dist, control_prior_dist, control_latents_dist,
                           cov_constr, treatment_constr, treatments,covariates)
            ...
            return {
                "Indiv-spec NLLH": indiv_spec_nllh.item(),
                # "Covar-spec NLLH": covar_spec_nllh.item(),
                "treatment_loss":treatment_loss.item(),
    ...
    def init_encoder_XT_prior(self):
        return MLP([self.covariate_dim + self.treatment_dim]
            + [self.hparams["encoder_width"]] * (self.hparams["encoder_depth"] - 1)
    ...
    def init_decoder_interv_concat(self):
        return MLP([(self.hparams["ZT_dim"])*2]
            + [self.num_treatments],
            heads=1, final_act= "softmax"
        )
    ...
    @torch.no_grad()
    def get_latent(self, outcomes, treatments, covariates, resample=False, sigma1=0.1, sigma2=0.1, sigma3=0.01):
        ...
        if resample:
            ZX_resample = self.sample_latent(ZX[0], sigma1*torch.ones_like(ZX[1], device="cuda"))
            ZXT_resample = self.sample_latent(ZXT[0],sigma2*torch.ones_like(ZXT[1], device="cuda"))
            ZT_resample = self.sample_latent(ZT[0], sigma3*torch.ones_like(ZT[1], device="cuda"))
        else:
            ZX_resample = ZX[0]
            ZXT_resample = ZXT[0]
            ZT_resample = ZT[0]
    ...
    @torch.no_grad()
    def get_latent_presentation(self, outcomes, treatments, covariates, sample=False,  sigma1=0.1, sigma2=0.1, sigma3=0.01):
        outcomes, treatments, covariates = self.move_inputs(
            outcomes, treatments, covariates
        )
        # if not sample:
        ZX, ZXT, ZT = self.get_latent(
                outcomes, treatments, covariates, sample, sigma1, sigma2, sigma3
            )
